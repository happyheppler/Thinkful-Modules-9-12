...
--In that induction, 17 players were posthumously inducted into the 
...
--Note that the year of induction was 2006, and the value for votedby 
--will be “Negro League.”
SELECT namefirst,
	namelast,
	birthyear,
	deathyear,
...
--fields from the salaries table, along with the category field from 
--the hof_inducted table. Keep only the records that are in both salaries 
--and hof_inducted. Hint: While a field named yearid is found in both 
--tables, don’t JOIN by it. You must, however, explicitly name which field
--to include.
...
--Write a query that returns the playerid, yearid, teamid, lgid, and salary 
...
--Write a query that returns the last name, first name (see people table), and total 
--recorded salaries for all players found in the salaries table.
SELECT namelast,
	namefirst,
...
--tables that include playerid, yearid, namefirst, and namelast. Hint: Each FROM 
--statement will include a LEFT OUTER JOIN!
...
--(in other words, the last name, followed by a comma, then a space, then the first 
--name). The query should also return the yearid and inducted fields. Include only 
--records since 1980 from both tables. Sort the resulting table by yearid, then inducted 
--so that Y comes before N. Finally, sort by the namefull field, A to Z.
...
FROM salaries
	...
	RIGHT OUTER JOIN max
		ON salaries.teamid = max.teamid
		AND salaries.yearid = max.yearid
		AND salaries.salary = max.max_salary
...
--Using the people table, write a query that returns namefirst, namelast, and 
--a field called usaborn where. The usaborn field should show the following: 
...
--Calculate the average height for players throwing with their right hand versus 
...
--Get the average of each team's maximum player salary since 2010. Hint: WHERE 
--will go inside your CTE.
WITH avg_max_salary AS
(
	SELECT teamid,
		yearid,
...
SELECT teamid,
	AVG(max_sal) AS avg_max_sal_since_2010
FROM avg_max_salary
GROUP BY teamid
ORDER BY teamid;